...
#Inputs:
#   1. cscpf: jeu de données où chaque ligne réprésente un polygone 
# (comme dans un fichier PEE_ORI) avec les colonnes: 
#         - GR_STATION: groupe de stations forestières
#         - HAUTEUR: numérique; ceci va être convertit en "M7M" ou "P7M"
#         - SDOM_BIO
...
#         - COURBE_V: classe de volume prédite par le même script ci-dessus
...
#   Ce tableau de correspondance sert à regrouper des combinaisons de 
...
#
#     
#   5. gtyfToTyf: tableau de conversion des GTYFs en TYFs pour nous aider à trouver
#   une courbe correspondante pour chaque GTYF. Ce tableau doit avoir des valeurs
#   de TYFs correspondantes à chaque combinaison SDOM_BIO, GR_STAT et GTYF
...
#                                     
#   7. clAgeToNum: tableau de conversion des classes d'âge dans des valeurs
#   numériques d'âge (e.g. CL_AGE 50 == 50)
...
#   conversión faite par François avec l'extrant de la prémière fonction. Ceci sert
#   à regrouper des groupes trop petits
#   
...
GE_M7M_ClassVol_Decal <- 
  function(cscpf, 
    ...
    #1.7 Objet "clAgeToNum" (table de correspondance entre les classe d'âge
    ...
    #2.2 Ajouter les GTYFs et les GR_STAT regroupés (pour enlever des groupes
    ...
    #Selon François on n'a pas besoin d'inclure le sous-domaine dans les
    #groupes. Ça va être fait après quand on décide la courbe à utiliser
    sumSup <- 
      cscpf %>% 
    ...
    #On va définir un seuil de superficie cumulative (défaut = 50%). Ça veut
    ...
    #dans des caractères pour éviter des avertissements inutiles
    classDecal <- 
      classDecal %>% 
